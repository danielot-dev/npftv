...
export async function createAlbum(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  let albumId: string;
  ...
  redirect(`/admin/gallery/${albumId}?success=Album+created+%E2%80%94+now+add+some+photos.`);
}
...
export async function updateAlbum(
  ...
  try {
    ...
    if (!current) return { error: "Album not found." };
  ...
  redirect("/admin/gallery?success=Album+updated.");
}
...
export async function addImageToAlbum(
  ...
  try {
    ...
    if (!url) {
      return { error: "Upload an image before adding it." };
    }
